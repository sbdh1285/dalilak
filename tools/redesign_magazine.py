...
<section class="home-hero"><div class="wrap"><div class="hero-grid"><div class="hero-copy"><span class="eyebrow">مجلة عربية عصرية</span><h1>{CONFIG['tagline']}</h1><p>{CONFIG['description']}</p><div class="cta-row"><a class="btn btn-a" href="category/home-tips.html">ابدأ القراءة</a><a class="btn btn-b" href="about.html">تعرّف على دليلك</a></div></div><div class="hero-media"><img src="{CONFIG['heroImage']}" alt="مكتب هادئ يضم كتبًا ودفترًا وأدوات ترمز إلى المعرفة والحياة اليومية" width="1440" height="900" fetchpriority="high"></div></div><div class="value-strip"><div class="value-item"><strong>محتوى واضح</strong><span>لغة مباشرة دون تعقيد</span></div><div class="value-item"><strong>أقسام متنوعة</strong><span>المنزل والطعام والمعرفة والتقنية</span></div><div class="value-item"><strong>قراءة مريحة</strong><span>تصميم عربي يركز على المحتوى</span></div><div class="value-item"><strong>تحديثات مستمرة</strong><span>مراجعة وتصحيح عند الحاجة</span></div></div></div></section>
<section class="sec"><div class="wrap"><div class="sec-h"><div class="sec-title"><span class="section-kicker">اختيار التحرير</span><h2>مقال مميز</h2></div></div><a class="featured" href="posts/{feat['slug']}.html"><img src="{feat['image']}" alt="{html.escape(feat['imageAlt'])}" width="1200" height="630"><div class="f-body"><span class="f-tag">{feat['category']}</span><h2>{feat['title']}</h2><p>{feat['description']}</p><span class="read-link">اقرأ المقال ←</span></div></a></div></section>
...
def redesign_post(path:Path,records:dict[str,dict])->None:
    ...
    text=re.sub(r'<img class="art-img"[^>]+>',f'<img class="art-img" src="../{a["image"]}" alt="{html.escape(a["imageAlt"])}" width="1200" height="630">',text,count=1)
