...
def guide_page(slug,title,desc,category,article_slugs,records):
 ...
 for s in article_slugs:
  a=records[s];items.append(f'<a class="guide-story" href="posts/{s}.html"><img src="{a["image"]}" alt="{html.escape(a["alt"])}" width="400" height="210" loading="lazy"><div><span>{a["category"]}</span><h2>{a["title"]}</h2><p>{a["description"]}</p></div></a>')
